...
class MemberInvestMentController extends MemberController
{
    ...
    public function get_package_list()
    {
        ...
        $response   = Tbl_investment_package::where(function($response) use($membership){
            $response->where('bind_membership',$membership)
    ...
    public function get_investment_list()
    {
        ...
        $slot_id            = Request::input('slot_id');
        $investment_list    = Tbl_investment_package_tag::where('user_id',Request::user()->id)->where('slot_id',$slot_id)
                              ->leftjoin('tbl_investment_package','tbl_investment_package.investment_package_id','=','tbl_investment_package_tag.investment_package_id')->get();
        ...
        {
            $logs                                                   = Tbl_investment_package_logs::where('investment_package_tag_id',$list->investment_package_tag_id);
            $last_logs                                              = $logs->orderBy('investment_package_logs_date','DESC')->value('investment_package_logs_date');
            // $current_last_logs                                      = $logs->orderBy('investment_package_logs_date','DESC')->where('investment_package_logs_amount','!=',0)->first();
            // $current_last_log                                       = $current_last_logs == null ? $list->investment_date : $current_last_logs->investment_package_logs_date;
            ...
            $last_log                                               = date($last_logs);
            $current_log                                            = date($last_logs) <= Carbon::now() ? date($last_logs)  : date(Carbon::now());

            
            $investment_list[$key]['remaining_days']                = date_create($current_log)->diff(date_create($last_log))->format('%a day(s) %h hrs %i min');
