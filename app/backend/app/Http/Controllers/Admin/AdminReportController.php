...
class AdminReportController extends AdminController
{
	...
	public function members_detail()
	{
		...
		foreach ($response as $key => $value)
		{
			...
			$response[$key]->address_info 		=  $address_list["address_info"];
			$response[$key]->barangay_city 		=  $address_list["brgyDesc"];
			$response[$key]->city 				=  $address_list["citymunDesc"];
			$response[$key]->region_province 	=  $address_list["provDesc"];
