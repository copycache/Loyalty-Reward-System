...
class AdminExportController extends AdminController
{
	...
	public function export_qoute_request_csv()
	{
		...
		$excels['data']  =   ['Item Name','Name','Email','Phone','Message'];
        Excel::create('QOUTE REQUEST', function($excel) use ($excels)
        {
            $excel->sheet('template', function($sheet) use ($excels)
            {
                ...
                {
                	$key = $key+=2;
                	$sheet->setCellValue('A'.$key, $list->item_sku);
                	$sheet->setCellValue('B'.$key, $list->qoute_request_name);
                    $sheet->setCellValue('C'.$key, $list->qoute_request_email);
                    $sheet->setCellValue('D'.$key, $list->qoute_request_phone);
                    $sheet->setCellValue('E'.$key, $list->qoute_request_message);
                }
            });
        })->download('xls');
	}
	...
	public function slot_wallet_history_csv()
	{
		...
		Excel::create("slot_wallet_history", function($excel) use ($data)
		{
		$excel->sheet("slot_wallet_history", function($sheet) use ($data)
		{
			$sheet->setOrientation('landscape');
			$sheet->loadView('export.csv.slot_wallet_history_csv', $data);
		});
			})->export('csv');
		// return $asd;
	}
	...
	public function slot_payout_history_csv()
	{
		...
		$prime['from'] = Request::input('from');
		$prime['to'] = Request::input('to');
		// $data = Self::slot_payout_history($prime);
        ...
        Excel::create("slot_payout_history" . "_" . (isset($data["_payout"][0]->slot_no) ? ($data["_payout"][0]->slot_no) : "slot_payout_history"), function($excel) use ($data)
        {
            ...
            {
                $sheet->setOrientation('landscape');
                $sheet->loadView('export.csv.slot_payout_history_csv', $data);
            });
	...
	public function export_cashin($ref)
	{
		...
		{
			$excels['_list'] = $data;
			$excels['data']  =   ['Slot Code','Member Name','Method','Currency','Amount Required','Cash In Charge','Cash In Status','Member will receive'];
	        Excel::create('CASH IN', function($excel) use ($excels)
	        {
	            $excel->sheet('template', function($sheet) use ($excels)
	            {
	                $data = $excels['data'];
	                ...
	                {
	                    $key = $key+=2;
	                    $sheet->setCellValue('A'.$key, $list['cash_in_slot_code']);
	                    $sheet->setCellValue('B'.$key, $list['cash_in_member_name']);
	                    $sheet->setCellValue('C'.$key, $list['cash_in_method_name']);
	                    $sheet->setCellValue('D'.$key, $list['cash_in_currency']);
	                    $sheet->setCellValue('E'.$key, $list['cash_in_payable']);
	                    $sheet->setCellValue('F'.$key, $list['cash_in_charge']);
	                    $sheet->setCellValue('G'.$key, $list['cash_in_status']);
	                    $sheet->setCellValue('H'.$key, $list['cash_in_receivable']);
	                }
	            });
	        })->download('xls');
		}
	...
	public function export_item_code_csv()
	{
		...
		$excels['_list'] = $response;
		$excels['data']  =   ['Code','Pin','Sold to','Transfer to','Used by'];
        Excel::create('ITEM CODE of '.strtoupper($item->item_sku), function($excel) use ($excels)
        {
            $excel->sheet('template', function($sheet) use ($excels)
            {
                $data = $excels['data'];
                ...
                foreach($excels['_list'] as  $key => $list)
                {
                    ...
                    $sheet->setCellValue('C'.$key, $list['code_org_buyer'] != null ? $list['code_org_buyer'] : "Unused");
                    $sheet->setCellValue('D'.$key, $list['code_buyer'] == null ? "Unused" : ($list['code_buyer']['name'] == $list['code_org_buyer'] ? '--' : $list['code_buyer']['name']) );
                    $sheet->setCellValue('E'.$key, $list['code_user'] != null ? $list['code_user']['name'] : "Unused");
                }
            });
        })->download('xls');
	}
	...
	public function export_payout_schedule_csv()
	{
		...
		$data['_bank']  =   ['Slot Code','Account Name','Account Number','Account Type','Email','Phone Number','TIN','Tax','Method Fee','Service Charge', 'Product Charge','GC Charge','Savings','Amount Due','Net Payout','Date'];
	...
	public function bonus_summary_csv()
	{
		...
		$output = Excel::create('Bonus summary as of '.$date_today, function($excel) use($search) {
			$excel->sheet('Top Recruiter', function($sheet) use($search) {
				...
				$sheet->appendRow(array(
					'Slot Code','Member Name','Member Contact #','Member Email','Direct Referral Income','Indirect Referral Income','Binary Income','Unilevel Bonus'
				));
				$key=2;
				...
				$query->chunk(100000, function($lists) use($sheet,&$key) {
					...
					foreach ($lists as $list)
					{
						// if($list['direct_income'] != 0 || $list['indirect_income'] != 0 || $list['binary_income'] != 0 || $list['unilevel_bonus_income'] != 0 )
						// {
							$sheet->setCellValue('A'.$key, $list['slot_no']);
							$sheet->setCellValue('B'.$key, $list['name']);
							$sheet->setCellValue('C'.$key, $list['contact']);
