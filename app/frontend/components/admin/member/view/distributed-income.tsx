...
const defaultFilters: DistributedFilters = { type: "all", from: null, to: null, search: null };
...
export function DistributedTab({ slot, active }: DistributedTabProps) {
  const { token } = useAuthStore();
  const [filters, setFilters] = useState<DistributedFilters>(defaultFilters);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    setData(null);
    apiPost<any>("/api/member/get_slot_distributed", { ...filters, id: slot.slot_id }, token)
      ...
      {!data ? <LoadingSpinner /> : (
        <div className="border rounded-lg overflow-hidden">
          <Table>
            ...
            <TableBody>
              {(data.data ?? []).length > 0 ? (data.data ?? []).map((d: any, i: number) => (
                <TableRow key={i}>
            ...
            {data.total_distributed != null && (
              <tfoot>
                <TableRow>
                  <TableHead colSpan={5} className="text-right">Total Distributed Income</TableHead>
                  <TableHead className="text-right font-bold text-blue-600">{Number(data.total_distributed).toFixed(2)}</TableHead>
                </TableRow>
              </tfoot>
            )}
          </Table>
