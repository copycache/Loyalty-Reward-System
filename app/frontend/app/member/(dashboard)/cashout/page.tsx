...
export default function MemberCashoutPage() {
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      const [userInfoRes, addressRes, methodRes, pendingRes, allTxRes] = await Promise.all([
        apiPost("/api/settings/get_user_info", {}, token).catch(() => null),
        apiPost("/api/settings/get_addresses", {}, token).catch(() => null),
        apiPost("/api/cashout/get_method_list", {}, token).catch(() => null),
        apiPost("/api/cashout/get_transactions", { cash_out_status: "pending/processing", slot_id: slotId }, token).catch(() => null),
        apiPost("/api/cashout/get_transactions", { cash_out_status: "all", slot_id: slotId }, token).catch(() => null),
      ]);
      ...
      if (Array.isArray(methodRes) && methodRes.length > 0) {
        ...
        selectMethod(defaultMethod, addressRes);
      }
    } catch { /* ignore */ }
    setLoading(false);
  ...
  const selectMethod = async (method: CashOutMethod, addrs?: Address[] | null) => {
    ...
    setCashOutAmount(0);
    setPrimaryInfo(currentSlot?.name || "");
    setEmailAddress(currentSlot?.email || "");
    setContactNumber(currentSlot?.contact || "");
    setSecondaryInfo("");
    setOptionalInfo(method.cash_out_method_category === "bank" ? "Savings" : null);

    // Load wallet for this method's currency
    try {
      const walletRes = await apiPost("/api/cashout/get_slot_wallet", {
        slot_id: slotId,
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await apiPost("/api/cashout/record_cash_out", {
        slot_id: slotId,
  ...
  const submitTin = async () => {
    ...
    try {
      const res = await apiPost(endpoint, payload, token);
      if (res?.status === "success") {
        toast.success(res.message || "TIN updated!");
        setTinDialogOpen(false);
        // Refresh user info
        const info = await apiPost("/api/settings/get_user_info", {}, token);
